...
{
    QPushButton *button = static_cast<QPushButton *>(this->button(id));
    if (button) {
        if (popupmode == InstantPopup) {
            button->setMenu(menu);
        } else {
            // implementation of KPushButton::setDelayedMenu(QMenu*)
            auto timer = new QTimer(button);
            timer->setSingleShot(true);
            connect(timer, &QTimer::timeout, [button, menu]() {
                    button->setMenu(menu);
                });
            const int delay = button->style()->styleHint(QStyle::SH_ToolButton_PopupDelay, nullptr, button);
            timer->start(delay <= 0 ? 150 : delay);
        }
    }
}
