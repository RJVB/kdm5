...
if (KDM5_KRB5AUTH)
    ...
    if (KRB5_LIBRARIES)
        ...
        if (KRB5_INCLUDE_DIR)
            ...
            message(STATUS "KDM5_KRB5AUTH requires Kerberos5 header files.
...
endif (KDM5_KERBEROS4)

if (KDM5_XDMCP AND X11_Xdmcp_FOUND)
    set(XDMCP 1)
endif (KDM5_XDMCP AND X11_Xdmcp_FOUND)

macro_bool_to_01(KDM5_XCONSOLE WITH_KDM_XCONSOLE)
...
FOREACH(path /usr/local/bin /usr/bin /bin ${CMAKE_INSTALL_PREFIX}/bin)
    if (XBINDIR STREQUAL ${path})
        set(dont_add_xbin 1)
    endif (XBINDIR STREQUAL ${path})
ENDFOREACH(path)
if (dont_add_xbin)
    set(KDM_DEF_USER_PATH "/usr/local/bin:/usr/bin:/bin:${CMAKE_INSTALL_PREFIX}/bin:/usr/games")
    set(KDM_DEF_SYSTEM_PATH "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:${CMAKE_INSTALL_PREFIX}/bin")
else (dont_add_xbin)
    set(KDM_DEF_USER_PATH "/usr/local/bin:/usr/bin:/bin:${XBINDIR}:${CMAKE_INSTALL_PREFIX}/bin:/usr/games")
    set(KDM_DEF_SYSTEM_PATH "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:${XBINDIR}:${CMAKE_INSTALL_PREFIX}/bin")
endif (dont_add_xbin)
...
if (NOT LIBEXEC_INSTALL_DIR STREQUAL "${BIN_INSTALL_DIR}")
    ...
    if (NOT inip)
        ...
        if (${LIBEXEC_INSTALL_DIR} MATCHES "^/")
            set(KDM_LIBEXEC_PATH "${LIBEXEC_INSTALL_DIR}")
            message(STATUS "Set KDM_LIBEXEC_PATH to LIBEXEC_INSTALL_DIR=${LIBEXEC_INSTALL_DIR}")
        else()
            set(KDM_LIBEXEC_PATH "${CMAKE_INSTALL_PREFIX}/${LIBEXEC_INSTALL_DIR}")
            message(STATUS "Computed absolute KDM_LIBEXEC_PATH=${KDM_LIBEXEC_PATH} from LIBEXEC_INSTALL_DIR=${LIBEXEC_INSTALL_DIR}")
        endif()
    else()
        ...
        STRING(SUBSTRING "${LIBEXEC_INSTALL_DIR}" ${ipsl} ${klxsfxl} KDM_LIBEXEC_PATH)
    endif()
endif()
